 ...
 这是一个**面向代码变更的多智能体回归测试用例生成**原型系统（LangGraph + FastAPI + OpenAI）。MVP 目标是跑通端到端链路：输入 `git diff` → 影响分析 → 生成 `pytest` 测试代码 → API 返回结构化结果。
...
```
 ...
 返回内容包含：受影响范围（impacted）、测试计划（test_plan）、生成的测试文件（generated_tests）。
