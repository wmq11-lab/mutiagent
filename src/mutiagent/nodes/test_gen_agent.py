...
def _llm_generate(state: WorkflowState) -> list[GeneratedTestFile]:
    ...
    system = (
        ...
        "21. **去重用例墙**：对同一 API/构造函数/CLI 入口的一大批**仅参数不同**的场景，必须用 ``@pytest.mark.parametrize``"
        " 或 **单测内表格驱动** 合并，**禁止**输出数十个结构雷同的独立 ``def test_*``；优先覆盖 **本次 diff** 与 **structured_test_plan 中 P0/P1** 条目，"
        "次要参数组合可省略。\n\n"
        "# 正确示例（必须遵循）\n\n"
        "## 示例1：异步API超时测试（正确写法）\n"
        "```python\n"
